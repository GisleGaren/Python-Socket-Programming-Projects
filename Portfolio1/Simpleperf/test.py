...
def run_client(server_ip, port, duration, unit, interval, parallel, numbytes):
	...
	total_data_sent = 0
	data_chunk = b'\0' * 1000 # Create a bytes object that contains 1000 null bytes. Each byte is "00000000" meaningless information but we just
	# need to define 1000 bytes to make sure 1000 bytes are being transfered each time.

	while time.time() - start_time < duration:  # Duration is defined in the -t part of the terminal.
		# As long as the difference between the start timer (start_time) and the timer that continues (time.time()) is smaller than the duration -t keep sending data chunks of 1000 bytes.
		sent = client_socket.send(data_chunk) # We don't know precicely the frequency of the .send() function as it also varies depending on hardware, but typically in the milliseconds.
		total_data_sent += sent  # Keep adding up the bytes that are being sent and store it in the total_data_sent variable
	...
	def print_interval_stats(client_socket, server_ip, port, start_time, duration, interval, unit):
		...
		while time.time() - start_time < duration:
			...
			if elapsed_interval_time >= interval:
				interval_end_time = current_time
				sent_data = (total_data_sent / format_to_bytes(1, unit))
				interval_bandwidth = sent_data / elapsed_interval_time
